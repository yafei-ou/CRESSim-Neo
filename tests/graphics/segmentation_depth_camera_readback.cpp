...
MeshResourceDesc makeCubeMesh(float halfExtent)
{
    ...
    mesh.debugName = "SegmentationDepthCamera.CubeMesh";
...
int main()
{
    ...
    {
        CRESSIM_LOG_WARNING(
            "Skipping segmentation depth camera readback test because runtime initialization failed.");
        return 0;
    }
    ...
    {
        CRESSIM_LOG_WARNING(
            "Skipping segmentation depth camera readback test because GPU device is unavailable.");
        runtime.shutdown();
        return 0;
    }
    ...
    targetDesc.debugName   = "SegmentationDepthCamera.Target";
    ...
    {
        runtime.shutdown();
        CRESSIM_LOG_ERROR("Failed to create segmentation depth camera target.");
        return 1;
    }
    ...
    materialDesc.debugName = "SegmentationDepthCamera.Material";
    ...
    for (std::uint64_t frameIndex = 0u; frameIndex < 2u; ++frameIndex)
    {
        ...
        if (request.id == 0u)
        {
            CRESSIM_LOG_ERROR("Failed to queue segmentation depth readback request.");
            runtime.shutdown();
            return 1;
        }
        if (frameIndex == 0u)
    ...
    {
        CRESSIM_LOG_ERROR(
            "Expected valid segmentation and depth readback payloads for both frames.");
        return 1;
    }
    if (firstEvent.colorFormat != Diligent::TEX_FORMAT_R32_UINT ||
        secondEvent.colorFormat != Diligent::TEX_FORMAT_R32_UINT)
    {
        CRESSIM_LOG_ERROR("Expected R32_UINT segmentation payloads.");
        return 1;
    }

    const std::uint32_t firstCenterId = cressim::neo::tests::helpers::decodeUint32Pixel(
        firstEvent, firstEvent.width / 2u, firstEvent.height / 2u);
    const std::uint32_t secondCenterId = cressim::neo::tests::helpers::decodeUint32Pixel(
        secondEvent, secondEvent.width / 2u, secondEvent.height / 2u);
    const std::uint32_t firstCornerId =
        cressim::neo::tests::helpers::decodeUint32Pixel(firstEvent, 0u, 0u);
    ...
    if (firstCenterId != backCube.segmentationId)
    {
        CRESSIM_LOG_ERROR("Expected back cube segmentation id before front cube appears. got=",
                          firstCenterId, " expected=", backCube.segmentationId);
        return 1;
    }
    if (secondCenterId != frontCube.segmentationId)
    {
        CRESSIM_LOG_ERROR("Expected front cube segmentation id after it becomes visible. got=",
                          secondCenterId, " expected=", frontCube.segmentationId);
        return 1;
    }
    if (firstCornerId != 0u)
    {
        CRESSIM_LOG_ERROR("Expected cleared background segmentation id 0. got=", firstCornerId);
        return 1;
    }
    if (!(secondCenterDepth < firstCenterDepth))
    {
        CRESSIM_LOG_ERROR("Expected nearer front cube to reduce center depth. first=",
                          firstCenterDepth, " second=", secondCenterDepth);
        return 1;
    }

    CRESSIM_LOG_INFO("SegmentationDepth camera readback passed. ids=", firstCenterId, "->",
                     secondCenterId, " depths=", firstCenterDepth, "->", secondCenterDepth);
    return 0;
}
