...
TextureResourceDesc makeBrdfLutDesc()
{
    TextureResourceDesc desc{};
    desc.debugName = "ViewerIntegration.Ibl.BrdfLut";
    desc.width = kBrdfLutSize;
    desc.height = kBrdfLutSize;
    desc.mipLevelCount = 1u;
    ...
    auto &pixels = desc.subresources.front().pixelData;
    pixels.reserve(static_cast<std::size_t>(kBrdfLutSize) * kBrdfLutSize * 8u);

    for (std::uint32_t y = 0u; y < kBrdfLutSize; ++y)
    {
        const float roughness =
            (static_cast<float>(y) + 0.5f) / static_cast<float>(kBrdfLutSize);
        for (std::uint32_t x = 0u; x < kBrdfLutSize; ++x)
        {
            const float nDotV =
                (static_cast<float>(x) + 0.5f) / static_cast<float>(kBrdfLutSize);
            ...
            const float a = saturate((-1.04f) * a004 + rz);
            const float b = saturate((1.04f) * a004 + rw);
            appendRgba16f(pixels, {a, b, 0.0f});
        }
    }
...
bool assignEnvironmentIbl(cressim::neo::engine::World &world,
                          ...
                          std::uint32_t envIndex,
                          const cressim::neo::graphics::TextureHandle brdfLut)
{
...
int main(int argc, char **argv)
{
    ...
    const MeshHandle sphereMesh = resources.registerMesh(makeSphereMesh(0.45f, 24u, 16u));
    const MeshHandle capsuleMesh =
        resources.registerMesh(makeCapsuleMesh(0.28f, 0.52f, 24u, 6u, 2u));

    const auto brdfLut = resources.registerTexture(makeBrdfLutDesc());
    ...
    {
        if (!assignEnvironmentIbl(world, resources, envIndex, brdfLut))
        {
            runtime.shutdown();
            viewer.shutdown();
