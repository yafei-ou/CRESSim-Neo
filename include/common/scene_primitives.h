...
struct SceneLayoutDesc
{
    ...
    std::uint32_t maxRenderableObjectsPerEnv = 4096u; ///< Maximum number of renderable mesh objects in each environment.
...
{
    Diligent::IBuffer *positionsBuffer    = nullptr; ///< GPU buffer containing instance 3D position vectors.
    Diligent::IBuffer *orientationsBuffer = nullptr; ///< GPU buffer containing instance orientation quaternions.
    ...
    std::uint64_t bindingGeneration       = 0;       ///< Revision counter tracking buffer reallocation/rebind events.
};
