...
/// @brief Offscreen render target allocation, resizing, binding, viewport manipulation, and readback system interface.

namespace cressim::neo::gpu
{

/// @brief Subsystem managing offscreen render targets, multi-layer attachments, viewports, and pixel readbacks.
...
public:
    ...
    /// @brief Sets the normalized viewport for subsequent rendering operations targeting this binding.
    ...
    /// @brief Binds the render target attachments to the active graphics context and applies clear values.
    /// @param binding Target binding and layer range.
    ...
    /// @brief Unbinds the render target attachments and transitions textures for subsequent sampling/compute.
    ...
    /// @brief Queues an asynchronous readback copy of the specified render target's pixels to CPU host memory.
    ...
    virtual bool tryGetRenderTargetDepthTexture(GpuRenderTargetHandle target,
                                                Diligent::ITexture *&outTexture)        = 0;
    /// @brief Retrieves the Diligent shader resource view (SRV) for sampling this target in later passes.
