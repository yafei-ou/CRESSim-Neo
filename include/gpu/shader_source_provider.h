...
/// @brief Shader file resolution, include directory path management, and stream factory provisioning for HLSL compilation.
...
struct ShaderSourceConfig
{
    ...
    bool includeSourceDirectory = true;                   ///< Automatically adds `<sourceDirectory>/include` and its subdirectories to include paths.
    std::vector<std::filesystem::path> includeDirectories;///< Additional search directories checked in order when resolving shader header includes.
};

/// @brief Service providing filesystem-backed input stream factories for Diligent shader compilation.
...
public:
    ...
    /// @brief Compatibility shorthand constructor configuring `<shaderDirectory>/include` as the include search root.
    ...
    /// @brief Retrieves the Diligent input stream factory for compiling shaders with nested includes.
