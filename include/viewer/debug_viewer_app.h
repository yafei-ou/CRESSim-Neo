...
struct DebugViewerKeymap
{
    ...
    int cyclePresentedCameraNext =
        93; ///< Switch active presentation view to next camera (default: ]).
    int cyclePresentedProbePrevious =
        44;                            ///< Cycle previous probe or sensor attachment (default: ,).
...
struct DebugViewerAppDesc
{
    ...
    bool useFixedTimestep =
        false;                  ///< Enforce fixed time step rather than elapsed wall-clock delta.
...
struct DebugViewerCameraBinding
{
    common::EntityId cameraEntity =
        common::kInvalidEntityId;  ///< Entity ID of camera driven by fly controls.
...
{
    std::function<void(const common::FrameContext &, engine::Runtime &)>
        beforeTick{}; ///< Invoked before physics stepping and systems tick.
    std::function<void(const common::FrameContext &, engine::Runtime &)>
        afterTick{}; ///< Invoked after physics stepping and render pass dispatch.
};

/// @brief Standalone desktop application hosting GLFW windowing, user input, camera controllers,
/// and main rendering loop.
