...
/// @brief GPU scene layout constants, shadow view parameters, prepared camera uniforms, and visibility pair structures.
...
/// @brief Maximum number of local (point/spot) lights processed per environment during forward shading.
...
/// @brief Computes the global linear light index for the primary directional light in a given environment.
...
{
    std::uint32_t shadowMode      = static_cast<std::uint32_t>(GpuLightShadowMode::None); ///< Active shadow mode enum.
    std::uint32_t shadowViewIndex = kInvalidGpuSceneIndex;                                ///< Index into global local shadow view array.
...
{
    std::array<Diligent::float4x4, kLocalShadowMaxFaceCount> lightViewProjectionMatrices{}; ///< View-projection matrices for spot or 6 cube faces.
    Diligent::float4 lightPositionRange{};   ///< World-space light position (xyz) and attenuation range (w).
    Diligent::float4 lightDirection{};       ///< World-space light direction vector.
    Diligent::float2 shadowTexelSize{0.0f, 0.0f}; ///< Reciprocal shadow map dimensions (1/width, 1/height).
    float shadowNearPlane    = 0.0f;         ///< Near clipping plane distance.
    float shadowFarPlane     = 0.0f;         ///< Far clipping plane distance.
    std::uint32_t lightIndex = kInvalidGpuSceneIndex; ///< Associated light index in the scene light buffer.
...
/// @brief GPU uniform buffer structure holding transformed camera matrices, cascaded shadow maps, and culling ranges.
struct GpuPreparedCamera
{
    Diligent::float4x4 viewMatrix           = Diligent::float4x4::Identity(); ///< World-to-camera view transformation matrix.
    Diligent::float4x4 viewProjectionMatrix = Diligent::float4x4::Identity(); ///< Combined view-projection matrix.
    std::array<Diligent::float4x4, 4> lightViewProjectionMatrices{};          ///< View-projection matrices for up to 4 directional shadow cascades.
...
struct GpuBatchCameraMetadata
{
    ...
    std::uint32_t mainLightIndex    = kInvalidGpuSceneIndex;  ///< Primary directional light index for this camera's environment.
    std::uint32_t colorLayer        = 0u;                     ///< Destination color texture array layer.
    std::uint32_t shadowLayer       = kInvalidBatchCameraLayer;///< Destination shadow texture array layer.
    std::uint32_t reserved0         = 0u;                     ///< Reserved padding.
    std::uint32_t reserved1         = 0u;                     ///< Reserved padding.
    std::uint32_t reserved2         = 0u;                     ///< Reserved padding.
};

/// @brief Output record produced by GPU frustum culling representing a visible (object, camera) pair.
