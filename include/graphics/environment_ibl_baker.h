...
/// @brief Offline and runtime IBL convolution baker for generating diffuse irradiance and specular prefiltered cubemaps.
...
/// @brief Parameter tuning descriptor controlling spherical harmonics / Monte Carlo IBL baking resolutions.
struct EnvironmentIblBakeOptions
{
    std::uint32_t irradianceSize        = 16u;  ///< Resolution of baked diffuse irradiance cubemap faces.
    std::uint32_t specularSize          = 128u; ///< Resolution of base mip level for prefiltered specular cubemap.
    std::uint32_t specularMipCount      = 7u;   ///< Number of roughness mip levels generated for specular convolution.
    std::uint32_t irradianceSampleCount = 256u; ///< Monte Carlo hemisphere sample count per texel for diffuse irradiance.
    std::uint32_t specularSampleCount   = 128u; ///< Importance sampling sample count per texel/roughness for specular reflection.
    float intensity                     = 1.0f; ///< Global radiant intensity multiplier.
    float backgroundIntensity           = -1.0f;///< Background skybox display intensity (-1 inherits `intensity`).
};

/// @brief Convolves raw in-memory cubemap face images and registers IBL textures in the resource manager.
...
/// @return Populated EnvironmentIblDesc referencing the registered background, irradiance, and specular cubemaps.
...
/// @brief Loads cubemap face image files from disk, performs IBL convolution, and registers resulting textures.
/// @param resources Resource manager to receive generated textures.
