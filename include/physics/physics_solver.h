...
/// @brief GPU-accelerated XPBD physics solver, numerical configuration descriptors, and stepping interfaces.
...
struct ContactSolverSettings
{
    ...
    float manifoldMergeSlopMultiplier          = 4.0f;    ///< Multiplier for clustering contact manifold points.
    float softRelaxation                       = 0.95f;   ///< Relaxation coefficient for soft particle contact.
    float softMaxCorrectionPerIteration        = 0.05f;   ///< Maximum position correction clamp per iteration for soft contact.
    float restitutionVelocityThreshold         = 0.5f;    ///< Minimum relative impact velocity required to trigger restitution.
    float restitutionPenetrationSlopMultiplier = 2.0f;    ///< Slop multiplier above which restitution is suppressed.
    float rigidRestitutionVelocityThreshold    = 0.0f;    ///< Minimum velocity threshold for rigid-body restitution.
    float positionFrictionMinDistance          = 1.0e-5f; ///< Minimum separation distance threshold for applying positional friction.
};

/// @brief Numerical stabilization and clamp parameters for rigid-body position and velocity solvers.
///
/// Occupies 32 bytes to match its GPU uniform counterpart layout.
struct RigidSolverSettings
{
    ...
    float depenetrationMaxCorrectionPerIteration   = 0.10f; ///< Maximum depenetration position correction per iteration.
    float velocityPenetrationStiffness             = 1.0f;  ///< Penetration stiffness coefficient for velocity projection.
    float maxTranslationCorrectionPerIteration     = 0.01f; ///< Maximum translation clamp per iteration.
    float maxRotationCorrectionPerIteration        = 0.10f; ///< Maximum angular rotation correction clamp per iteration.
    float maxLinearVelocityCorrectionPerIteration  = 0.8f;  ///< Maximum linear velocity update clamp per iteration.
    float maxAngularVelocityCorrectionPerIteration = 0.5f;  ///< Maximum angular velocity update clamp per iteration.
};
...
struct JointSolverSettings
{
    ...
    float articulatedRelaxation    = 0.7f;    ///< Relaxation factor for hinge, slider, and spherical joints.
    float maxError                 = 0.05f;   ///< Maximum allowable joint position error before clamping.
    float maxTranslationCorrection = 0.02f;   ///< Maximum linear translation correction applied per iteration.
    ...
    float angularRegularization    = 5.0e-5f; ///< Regularization epsilon for angular constraint matrices.
...
{
    float internalRelaxation = 0.2f; ///< Relaxation factor for distance, bending, and tetrahedral volume constraints.
...
struct FluidSolverSettings
{
    ...
    float maxUnderDensityRatio = 0.0f;  ///< Threshold below rest density for allowing negative pressure.
    float reserved1            = 0.0f;  ///< Reserved padding.
    float reserved2            = 0.0f;  ///< Reserved padding.
    float reserved3            = 0.0f;  ///< Reserved padding.
};
...
struct PhysicsSolverDesc
{
    ...
    std::uint32_t fluidIterations             = 0;  ///< Fluid density constraint iterations (0 uses defaultIterations).
    std::uint32_t softInternalIterations      = 0;  ///< Soft-body internal constraint iterations (0 uses defaultIterations).
    std::uint32_t softContactIterations       = 0;  ///< Soft-body contact iterations (0 uses defaultIterations).
    std::uint32_t rigidJointIterations        = 0;  ///< Rigid joint constraint iterations (0 uses defaultIterations).
    std::uint32_t rigidRigidContactIterations = 0;  ///< Rigid-rigid contact iterations (0 uses defaultIterations).
    ...
    bool enableBlockingReadback = true;             ///< Whether each physics step blocks to copy GPU simulated state back to CPU host memory.
};
...
public:
    ...
    /// @brief Retrieves the current non-owning GPU buffer views for rigid, joint, soft, and curve scenes.
    /// @return PhysicsGpuSceneView containing pointers and descriptor counts.
    PhysicsGpuSceneView gpuSceneView() const noexcept;

    /// @brief Returns the shared export buffer holding particle positions and inverse mass for CUDA / renderer interop.
