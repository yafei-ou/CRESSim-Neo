...
struct CustomComputeResourceDesc
{
    ...
    CustomComputeResourceKind kind     = CustomComputeResourceKind::Buffer; ///< Resource type kind (Buffer, Texture).
...
struct CustomComputeResourceBindingDesc
{
    ...
    CustomComputeResourceAccess access = CustomComputeResourceAccess::ReadOnly; ///< Resource access mode.
};
...
{
    CustomComputeDispatchMode mode = CustomComputeDispatchMode::ExplicitGroupCount; ///< Dispatch mode.
...
struct CustomComputePassDesc
{
    ...
    std::vector<std::filesystem::path> includeDirectories; ///< Custom shader search paths for HLSL includes.
