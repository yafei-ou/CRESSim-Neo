...
constexpr float kFluidBottomY         = 1.2f;
constexpr float kDefaultFluidHeight   = 6.0f;
...
int main(int argc, char **argv)
{
    ...
    viewerDefaults.windowTitle = "CRESSim Neo Fluid Tall Block Stress";
    ...
    cameraTransform.worldTransform.position = {0.0f, 5.0f, -12.5f};
    cameraTransform.worldTransform.rotation =
        Diligent::QuaternionF::RotationFromAxisAngle({1.0f, 0.0f, 0.0f}, 0.28f);
    ...
    fluidMaterial.cohesion = 1.0f;
    fluidMaterial.gravityScale = 0.5f;
    fluidMaterial.cflCoefficient = 1.0f;
    fluidMaterial.vorticityConfinement = 1.0f;
    fluidMaterial.surfaceTension =  2.0f;
    ...
    if (!spawnFluid(world, fluidCenter, fluidSize, fluidMaterial, {0.18f, 0.66f, 0.95f, 0.72f}))
    {
        runtime.shutdown();
        viewer.shutdown();
        CRESSIM_LOG_ERROR("Failed to author tall fluid block.\n");
        return 1;
    }

    world.physicsWorld().ensureDerivedStateUpToDate();
    const auto &particles = world.physicsWorld().particles();
    CRESSIM_LOG_INFO("Fluid tall block stress authored ", particles.size(),
                     " particles. Fluid height=", options.fluidHeight,
                     ", fluid size=(", fluidSize.x, ", ", fluidSize.y, ", ", fluidSize.z,
                     "), fluid bodies=", world.physicsWorld().fluidCount(), ".\n");
