...
enum class EnvLightMode : std::uint32_t
{
    DirectionalWithShadows = 0u,
    DirectionalNoShadows   = 1u,
    NoDirectionalLight     = 2u,
};

struct EnvDirectionalLightAuthoring
{
...
EnvLightMode lightModeForEnv(std::uint32_t envIndex)
{
    switch (envIndex % 3u)
    {
    case 0u:
        return EnvLightMode::DirectionalWithShadows;
    case 1u:
        return EnvLightMode::DirectionalNoShadows;
    default:
        return EnvLightMode::NoDirectionalLight;
    }
}

EnvDirectionalLightAuthoring directionalLightForEnv(std::uint32_t envIndex)
{
    const float phase = static_cast<float>(envIndex / 3u) * 0.95f;
    const float shadowAzimuth = -2.2f + phase;
    const float noShadowAzimuth = 0.8f + phase;

    switch (lightModeForEnv(envIndex))
    {
    case EnvLightMode::DirectionalWithShadows:
        return EnvDirectionalLightAuthoring{
            Diligent::normalize(
                Diligent::float3{std::cos(shadowAzimuth), -1.0f, std::sin(shadowAzimuth)}),
            {1.0f, 0.94f, 0.88f},
            8.0f,
            true};
    case EnvLightMode::DirectionalNoShadows:
        return EnvDirectionalLightAuthoring{
            Diligent::normalize(
                Diligent::float3{1.35f * std::cos(noShadowAzimuth), -0.32f,
                                 1.35f * std::sin(noShadowAzimuth)}),
            {1.0f, 0.94f, 0.88f},
            8.0f,
            false};
    case EnvLightMode::NoDirectionalLight:
        break;
    }
                       ...
                       cressim::neo::common::EntityId &outCameraEntity)
{
    ...
    const EnvLightMode lightMode = lightModeForEnv(envIndex);
    if (lightMode != EnvLightMode::NoDirectionalLight)
    {
        const auto lightEntity = world.createEntity(envIndex);
        const EnvDirectionalLightAuthoring authoredLight = directionalLightForEnv(envIndex);
        DirectionalLightComponent light{};
        light.direction = authoredLight.direction;
        light.color = authoredLight.color;
        light.intensity = authoredLight.intensity;
        light.castsShadows = authoredLight.castsShadows;
    ...
    for (int layer = 0; layer < kLayers; ++layer)
    {
        for (int z = 0; z < kGridDepth; ++z)
        {
            for (int x = 0; x < kGridWidth; ++x)
            {
                ...
                body.linearVelocity = {static_cast<float>((x % 3) - 1) * 0.08f + envVelocityBiasX,
                                       0.0f,
...
int main(int argc, char **argv)
{
    ...
    viewerDesc.windowTitle = "CRESSim Neo Physics Viewer Large Array Multi Env Mixed Lighting";
    ...
    boxMaterialDesc.debugName = "ViewerIntegration.MixedLighting.BoxMaterial";
    ...
    sphereMaterialDesc.debugName = "ViewerIntegration.MixedLighting.SphereMaterial";
    ...
    capsuleMaterialDesc.debugName = "ViewerIntegration.MixedLighting.CapsuleMaterial";
    ...
    planeMaterialDesc.debugName = "ViewerIntegration.MixedLighting.GroundMaterial";
    planeMaterialDesc.baseColor = {0.72f, 0.74f, 0.77f};
    planeMaterialDesc.metallic = 0.0f;
    planeMaterialDesc.roughness = 0.90f;

    const EnvMaterialSet sharedMaterials{
    ...
    CRESSIM_LOG_INFO("Physics viewer large array multi-env mixed-lighting passed. Envs=",
                     envCount, " Frames=", viewerDesc.maxFrames, '\n');
    return 0;
}
