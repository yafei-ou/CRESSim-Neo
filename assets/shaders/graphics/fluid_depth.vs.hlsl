...
#define g_FluidDepthSceneDepthLayer g_FluidDepthParams.y
#define g_FluidDepthEnvIndex g_FluidDepthParams.z
...
CRESSIM_STRUCTURED_BUFFER(float4, g_FluidAnisotropy1);
CRESSIM_STRUCTURED_BUFFER(float4, g_FluidAnisotropy2);
CRESSIM_STRUCTURED_BUFFER(float4, g_FluidAnisotropy3);
...
void main(uint vertexId : SV_VertexID, out VSOutput Out)
{
    ...
    const PreparedCamera preparedCamera = CRESSIM_SB_LOAD(g_PreparedCameras, g_FluidDepthCameraIndex);
    ...
    if (preparedCamera.active == 0u || cameraInput.active == 0u ||
        particleEnvIndex != g_FluidDepthEnvIndex || particleKind != 1u)
    {
        Out.Position = float4(2.0, 2.0, 2.0, 1.0);
        Out.ViewPos = float3(0.0, 0.0, 1.0);
        Out.Uv = float2(0.0, 0.0);
        Out.Ani1 = float4(1.0, 0.0, 0.0, 1.0);
        Out.Ani2 = float4(0.0, 1.0, 0.0, 1.0);
        Out.Ani3 = float4(0.0, 0.0, 1.0, 1.0);
        Out.AniScale = 1.0;
        Out.SceneDepthLayer = g_FluidDepthSceneDepthLayer;
        return;
    }
    ...
    const float3x3 viewRotation = (float3x3)preparedCamera.viewMatrix;
    float4 ani1 = CRESSIM_SB_LOAD(g_FluidAnisotropy1, particleIndex);
    float4 ani2 = CRESSIM_SB_LOAD(g_FluidAnisotropy2, particleIndex);
    float4 ani3 = CRESSIM_SB_LOAD(g_FluidAnisotropy3, particleIndex);

    ani1 = float4(mul(ani1.xyz, viewRotation), max(ani1.w, 1.0e-4));
    ani2 = float4(mul(ani2.xyz, viewRotation), max(ani2.w, 1.0e-4));
    ani3 = float4(mul(ani3.xyz, viewRotation), max(ani3.w, 1.0e-4));
    const float scale = max(max(ani1.w, ani2.w), ani3.w) * 1.5;
    ani1.w /= scale;
    ani2.w /= scale;
    ani3.w /= scale;
