...
{
    const uint totalSubviewCount = localShadowTotal2DSubviewCount(g_EnvCount);
    ...
    if (totalSubviewCount == 0u || totalObjectCount == 0u ||
        globalIndex >= totalSubviewCount * totalObjectCount)
    {
        return;
    }

    const uint subviewIndex = globalIndex / totalObjectCount;
    const uint objectIndex = globalIndex % totalObjectCount;
    const uint envIndex = objectIndex / g_MaxObjectsPerEnv;
    if (envIndex != localShadow2DSubviewIndexToEnv(subviewIndex))
    {
        return;
    }
    ...
    const uint shadowViewIndex = localShadow2DViewIndex(envIndex,
                                                        localShadow2DSubviewIndexToSlot(subviewIndex));
    const LocalShadowView shadowView = CRESSIM_SB_LOAD(g_LocalShadowViews, shadowViewIndex);
    if (shadowView.active == 0u || shadowView.layerCount == 0u)
    {
        return;
    }
    ...
    if (!valid || !localShadowMatrixIntersectsCorners(corners, shadowView.lightViewProjectionMatrices[0]))
    {
        return;
    }

    const uint commandIndex = queueInfo.localShadowCommandIndex * totalSubviewCount + subviewIndex;
