...
CRESSIM_STRUCTURED_BUFFER(GpuSurfaceChunkRange, g_SurfaceChunkRanges);
CRESSIM_STRUCTURED_BUFFER(GpuBodyAabb, g_SurfaceChunkAabbs);
CRESSIM_RW_STRUCTURED_BUFFER(GpuBodyAabb, g_SurfaceWorldAabbsRW);
...
{
    const uint surfaceIndex = groupId.x;
    if (surfaceIndex >= surfaceCount)
    {
        return;
    }

    const GpuSurfaceChunkRange range = CRESSIM_SB_LOAD(g_SurfaceChunkRanges, surfaceIndex);
    ...
    if (range.count == 0u)
    {
        if (lane == 0u)
        {
            ...
            CRESSIM_SB_STORE(g_SurfaceWorldAabbsRW, surfaceIndex, bodyAabb);
        }
        return;
    }
    ...
    {
        const GpuBodyAabb chunkAabb = CRESSIM_SB_LOAD(g_SurfaceChunkAabbs, range.start + i);
