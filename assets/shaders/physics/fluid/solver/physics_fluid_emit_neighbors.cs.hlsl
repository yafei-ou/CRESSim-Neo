...
void main(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    const uint candidateOffset = CRESSIM_SB_LOAD(g_CandidateOffsets, particleIndex);
    uint candidateCount = CRESSIM_SB_LOAD(g_CandidateCounts, particleIndex);
    if (candidateOffset >= fluidNeighborPairCapacity)
    {
        CRESSIM_SB_STORE(g_CandidateCounts, particleIndex, 0u);
        return;
    }

    // This path currently clamps to the transient buffer capacity and can silently
    // truncate dense fluid neighborhoods. Keep that behavior for now until we add
    // a dedicated overflow meta/reporting path.
    candidateCount = min(candidateCount, fluidNeighborPairCapacity - candidateOffset);
        ...
        for (int dy = -1; dy <= 1; ++dy)
        {
            ...
            for (int dx = -1; dx <= 1; ++dx)
            {
                ...
                for (uint sortedIndex = range.startIndex; sortedIndex < range.endIndex; ++sortedIndex)
                {
                    ...
                    if (writeIndex >= candidateOffset + candidateCount)
                    {
                        return;
                    }
