...
MeshGpuCache::MeshGpuCache(std::string debugPrefix) :
    mDebugPrefix(std::move(debugPrefix))
{
}

MeshGpuCache::CachedBuffers* MeshGpuCache::getOrCreate(const ForwardDrawCommand& drawCommand, Diligent::IRenderDevice* renderDevice)
{
    ...
    const bool recreate = mesh.vertexBuffer == nullptr || mesh.indexBuffer == nullptr || mesh.version != drawCommand.meshVersion;
    if (!recreate)
    {
        return &mesh;
    }
