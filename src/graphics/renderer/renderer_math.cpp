...
{
    const Diligent::QuaternionF rotation = common::runtime_math::normalizeQuaternion(cameraTransform.rotation);
    ...
    const Diligent::float3 xAxis = common::runtime_math::safeNormalize(Diligent::cross(up, zAxis), Diligent::float3{1.0f, 0.0f, 0.0f});
    const Diligent::float3 yAxis = common::runtime_math::safeNormalize(Diligent::cross(zAxis, xAxis), Diligent::float3{0.0f, 1.0f, 0.0f});
...
{
    const Diligent::float3 zAxis = common::runtime_math::safeNormalize(at - eye, Diligent::float3{0.0f, 0.0f, 1.0f});
    const Diligent::float3 xAxis = common::runtime_math::safeNormalize(Diligent::cross(up, zAxis), Diligent::float3{1.0f, 0.0f, 0.0f});
    const Diligent::float3 yAxis = common::runtime_math::safeNormalize(Diligent::cross(zAxis, xAxis), Diligent::float3{0.0f, 1.0f, 0.0f});
    const Diligent::float4x4 viewRotation = Diligent::float4x4::ViewFromBasis(xAxis, yAxis, zAxis);
    const Diligent::float4x4 viewTranslation = Diligent::float4x4::Translation(-eye);
...
CameraFrustumInfo buildCameraFrustumInfo(const CameraData& camera, float outputWidth, float outputHeight)
{
    ...
    info.aspect = common::runtime_math::clampPositive(outputWidth, 1.0f) / common::runtime_math::clampPositive(outputHeight, 1.0f);
    info.fovRadians = std::max(camera.verticalFovDegrees, 1.0f) * common::runtime_math::degreesToRadians(1.0f);
    info.nearPlane = std::max(camera.nearClip, 0.001f);
    info.farPlane = std::max(camera.farClip, info.nearPlane + 0.001f);

    const Diligent::QuaternionF rotation = common::runtime_math::normalizeQuaternion(camera.worldTransform.rotation);
    ...
    const Diligent::float3 worldUp = common::runtime_math::safeNormalize(
        rotation.RotateVector(Diligent::float3{0.0f, 1.0f, 0.0f}),
        Diligent::float3{0.0f, 1.0f, 0.0f});
    info.right = common::runtime_math::safeNormalize(Diligent::cross(worldUp, info.forward), Diligent::float3{1.0f, 0.0f, 0.0f});
    info.up = common::runtime_math::safeNormalize(Diligent::cross(info.forward, info.right), Diligent::float3{0.0f, 1.0f, 0.0f});
...
{
    const Diligent::QuaternionF rotation = common::runtime_math::normalizeQuaternion(transform.rotation);
