...
bool PhysicsPassDispatcher::writeSurfaceRenderDispatchConstants(
    Diligent::IDeviceContext *computeContext, const GpuSurfaceRenderDispatchConstants &constants)
{
    return writeConstantsBuffer(computeContext, mSurfaceRenderDispatchConstantsBuffer, &constants,
                                sizeof(constants));
}
...
bool PhysicsPassDispatcher::initialize(gpu::GpuDevice &device, std::uint32_t physicsContextId)
{
    ...
    if (!initPass(mRigidPredictPass, kPredictRigid) || !initPass(mSoftPredictPass, kSoftPredict) ||
        ...
        !initPass(mSkinSurfaceDeformableVerticesPass, kSkinSurfaceDeformableVertices) ||
        !initPass(mUpdateSurfaceTriangleNormalsPass, kUpdateSurfaceTriangleNormals) ||
        !initPass(mUpdateSurfaceDeformableNormalsPass, kUpdateSurfaceDeformableNormals) ||
        !initPass(mUpdateCurveRenderDataPass, kUpdateCurveRenderData) ||
        !initPass(mUpdateSurfaceBoundsPass, kUpdateSurfaceBounds) ||
        !initPass(mFinalizeSurfaceBoundsPass, kFinalizeSurfaceBounds) ||
        ...
        !initPass(mUpdateHingeJointRuntimeStatePass, kUpdateHingeJointRuntimeState) ||
        !initPass(mUpdateSliderJointRuntimeStatePass, kUpdateSliderJointRuntimeState) ||
        !initPass(mClearHingeJointConstraintStatePass, kClearHingeJointConstraintState) ||
        !initPass(mClearSphericalJointConstraintStatePass, kClearSphericalJointConstraintState) ||
    ...
    return createConstantsBuffer("CRESSimNeo.Physics.RigidDispatchConstants",
               ...
               createConstantsBuffer("CRESSimNeo.Physics.SurfaceRenderDispatchConstants",
                                     sizeof(GpuSurfaceRenderDispatchConstants),
                                     mSurfaceRenderDispatchConstantsBuffer) &&
...
bool PhysicsPassDispatcher::updateHingeJointRuntimeState(Diligent::IDeviceContext *computeContext,
                                                         const PhysicsSceneGpuState &sceneState,
                                                         std::uint32_t jointCount)
{
    if (jointCount == 0u)
    ...
    const std::array bindings{
        ...
        gpu::GpuBufferBinding{"g_HingeJoints", persistentJoints.hingeJointsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_HingeJointRuntimeStates",
                              persistentJoints.hingeJointRuntimeStatesBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    const GpuRigidJointDispatchConstants jointConstants{jointCount, 0u, 0u, 0u};
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           mUpdateHingeJointRuntimeStatePass.dispatch(computeContext, kDefaultVariant, bindings,
                                                      dispatchGroupCount(jointCount));
}

bool PhysicsPassDispatcher::updateSliderJointRuntimeState(Diligent::IDeviceContext *computeContext,
                                                          const PhysicsSceneGpuState &sceneState,
                                                          std::uint32_t jointCount)
{
    if (jointCount == 0u)
    ...
    const std::array bindings{
        ...
        gpu::GpuBufferBinding{"g_SliderJoints", persistentJoints.sliderJointsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SliderJointRuntimeStates",
                              persistentJoints.sliderJointRuntimeStatesBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    const GpuRigidJointDispatchConstants jointConstants{jointCount, 0u, 0u, 0u};
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           mUpdateSliderJointRuntimeStatePass.dispatch(computeContext, kDefaultVariant, bindings,
                                                       dispatchGroupCount(jointCount));
}

bool PhysicsPassDispatcher::clearSphericalJointConstraintState(
    Diligent::IDeviceContext *computeContext, const PhysicsSceneGpuState &sceneState,
    std::uint32_t jointCount)
{
    if (jointCount == 0u)
    ...
    const std::array bindings{
        gpu::GpuBufferBinding{"PhysicsRigidJointDispatchConstantsBuffer",
                              mRigidJointDispatchConstantsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SphericalJointTranslationLambdas",
                              transient.sphericalJointTranslationLambdasBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
        gpu::GpuBufferBinding{"g_SphericalJointRotationLambdas",
                              transient.sphericalJointRotationLambdasBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    const GpuRigidJointDispatchConstants jointConstants{jointCount, 0u, 0u, 0u};
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           mClearSphericalJointConstraintStatePass.dispatch(
    ...
    std::uint32_t routedCableCount, const GpuRigidDispatchConstants &constants)
{
    ...
    return writeRigidDispatchConstants(computeContext, constants) &&
           mClearRoutedCableConstraintStatePass.dispatch(computeContext, kDefaultVariant, bindings,
...
bool PhysicsPassDispatcher::skinSurfaceDeformableVertices(Diligent::IDeviceContext *computeContext,
                                                          const PhysicsSceneGpuState &sceneState,
                                                          std::uint32_t renderVertexCount)
{
    ...
    const GpuSurfaceRenderDispatchConstants constants{renderVertexCount, 0u, 0u, 0u};
    const auto &softParticles = sceneState.persistentParticles();
    const auto &softTopology  = sceneState.persistentSoftTopology();
    const std::array bindings{
        gpu::GpuBufferBinding{"PhysicsSurfaceRenderDispatchConstantsBuffer",
                              mSurfaceRenderDispatchConstantsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_ParticlePositionsInvMass", softParticles.positionsInvMassBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderVertexBindings",
                              softTopology.renderVertexBindingsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderPositionsRW",
                              softTopology.surfaceRenderPositionsBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    return writeSurfaceRenderDispatchConstants(computeContext, constants) &&
           mSkinSurfaceDeformableVerticesPass.dispatch(computeContext, kDefaultVariant, bindings,
                                                       dispatchGroupCount(renderVertexCount));
}

bool PhysicsPassDispatcher::updateSurfaceTriangleNormals(Diligent::IDeviceContext *computeContext,
                                                         const PhysicsSceneGpuState &sceneState,
                                                         std::uint32_t renderTriangleCount)
{
    ...
    const GpuSurfaceRenderDispatchConstants constants{0u, renderTriangleCount, 0u, 0u};
    const auto &softParticles = sceneState.persistentParticles();
    const auto &softTopology  = sceneState.persistentSoftTopology();
    const std::array bindings{
        gpu::GpuBufferBinding{"PhysicsSurfaceRenderDispatchConstantsBuffer",
        ...
        gpu::GpuBufferBinding{"g_ParticlePositionsInvMass", softParticles.positionsInvMassBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderTriangleParticleIndices",
                              softTopology.renderTriangleParticleIndicesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderTriangleNormalsRW",
                              softTopology.renderTriangleNormalsBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    return writeSurfaceRenderDispatchConstants(computeContext, constants) &&
           mUpdateSurfaceTriangleNormalsPass.dispatch(computeContext, kDefaultVariant, bindings,
                                                      dispatchGroupCount(renderTriangleCount));
}

bool PhysicsPassDispatcher::updateSurfaceDeformableNormals(Diligent::IDeviceContext *computeContext,
                                                           const PhysicsSceneGpuState &sceneState,
                                                           std::uint32_t renderVertexCount)
{
    if (renderVertexCount == 0u)
    ...
    const std::array bindings{
        gpu::GpuBufferBinding{"PhysicsSurfaceRenderDispatchConstantsBuffer",
                              mSurfaceRenderDispatchConstantsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderTriangleNormals",
                              softTopology.renderTriangleNormalsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderVertexTriangleRanges",
                              softTopology.renderVertexTriangleRangesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderVertexTriangleIndices",
                              softTopology.renderVertexTriangleIndicesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderFallbackNormals",
                              softTopology.surfaceFallbackNormalsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceRenderNormalsRW", softTopology.surfaceRenderNormalsBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    return writeSurfaceRenderDispatchConstants(computeContext, constants) &&
           mUpdateSurfaceDeformableNormalsPass.dispatch(computeContext, kDefaultVariant, bindings,
                                                        dispatchGroupCount(renderVertexCount));
}
...
bool PhysicsPassDispatcher::updateSurfaceDeformableBounds(Diligent::IDeviceContext *computeContext,
                                                          const PhysicsSceneGpuState &sceneState,
                                                          std::uint32_t surfaceCount,
                                                          std::uint32_t surfaceBoundsChunkCount)
{
    if (surfaceCount == 0u || surfaceBoundsChunkCount == 0u)
    {
        return true;
    }

    const GpuSurfaceRenderDispatchConstants chunkConstants{0u, 0u, 0u, surfaceBoundsChunkCount};
    const GpuSurfaceRenderDispatchConstants finalizeConstants{0u, 0u, surfaceCount, 0u};
    const auto &softParticles = sceneState.persistentParticles();
    const auto &softTopology  = sceneState.persistentSoftTopology();
    const auto &transient     = sceneState.transientBuffers();
    const std::array chunkBindings{
        ...
        gpu::GpuBufferBinding{"g_ParticlePositionsInvMass", softParticles.positionsInvMassBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceBoundsChunks", softTopology.surfaceBoundsChunksBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceChunkAabbsRW", transient.surfaceChunkAabbsBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };
    const std::array finalizeBindings{
        gpu::GpuBufferBinding{"PhysicsSurfaceRenderDispatchConstantsBuffer",
                              mSurfaceRenderDispatchConstantsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceChunkRanges", softTopology.surfaceChunkRangesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceChunkAabbs", transient.surfaceChunkAabbsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SurfaceWorldAabbsRW", softTopology.surfaceWorldAabbsBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    };

    return writeSurfaceRenderDispatchConstants(computeContext, chunkConstants) &&
           mUpdateSurfaceBoundsPass.dispatch(computeContext, kDefaultVariant, chunkBindings,
                                             surfaceBoundsChunkCount) &&
           writeSurfaceRenderDispatchConstants(computeContext, finalizeConstants) &&
           mFinalizeSurfaceBoundsPass.dispatch(computeContext, kDefaultVariant, finalizeBindings,
                                               surfaceCount);
}
...
bool PhysicsPassDispatcher::dispatchSolveSphericalJointConstraintsPass(
    Diligent::IDeviceContext *computeContext, const PhysicsSceneGpuState &sceneState,
    std::uint32_t jointCount)
{
    if (computeContext == nullptr)
    ...
    const std::array bindings{
        ...
        gpu::GpuBufferBinding{"g_RigidBodyTypes", persistentBodies.bodyTypesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SphericalJoints", persistentJoints.sphericalJointsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_SphericalJointTranslationLambdas",
                              transient.sphericalJointTranslationLambdasBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
    ...
    return mSolveSphericalJointConstraintsPass.dispatch(computeContext, kDefaultVariant, bindings,
                                                        dispatchGroupCount(jointCount));
}

bool PhysicsPassDispatcher::solveSphericalJointConstraints(Diligent::IDeviceContext *computeContext,
                                                           const PhysicsSceneGpuState &sceneState)
{
    const std::uint32_t jointCount = sceneState.sphericalJointCount();
    if (computeContext == nullptr)
    {
        return false;
    }
    ...
    const GpuRigidJointDispatchConstants jointConstants{jointCount, 0u, 0u, 0u};
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           dispatchSolveSphericalJointConstraintsPass(computeContext, sceneState, jointCount);
}
...
bool PhysicsPassDispatcher::solveRoutedCableConstraints(Diligent::IDeviceContext *computeContext,
                                                        const PhysicsSceneGpuState &sceneState,
                                                        ...
                                                        const GpuRigidDispatchConstants &constants)
{
    ...
    const std::array bindings{
        ...
        gpu::GpuBufferBinding{"g_RigidBodyTypes", persistentBodies.bodyTypesBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_RoutedCableConstraints", persistentCables.descriptorsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_RoutedCableRoutePoints", persistentCables.routePointsBuffer,
                              Diligent::BUFFER_VIEW_SHADER_RESOURCE},
        gpu::GpuBufferBinding{"g_RoutedCableLambdas", transient.routedCableLambdasBuffer,
                              Diligent::BUFFER_VIEW_UNORDERED_ACCESS},
        gpu::GpuBufferBinding{"g_RigidBodyTranslationCorrections",
    ...
    std::uint32_t constraintCount, const GpuRigidDispatchConstants &constants)
{
    ...
    dispatchConstants.reserved0                 = constraintCount;
    return writeRigidDispatchConstants(computeContext, dispatchConstants) &&
           mSolveRigidDistanceConstraintsPass.dispatch(computeContext, kDefaultVariant, bindings,
                                                       dispatchGroupCount(constraintCount));
}
...
bool PhysicsPassDispatcher::solveHingeJointTargetVelocities(
    Diligent::IDeviceContext *computeContext, const PhysicsSceneGpuState &sceneState)
{
    const std::uint32_t jointCount = sceneState.hingeDrivenJointCount();
    if (computeContext == nullptr)
    {
        return false;
    }
    ...
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           dispatchSolveHingeJointVelocityTargetsPass(
               computeContext, sceneState, persistentJoints.hingeDrivenJointIndicesBuffer,
               jointCount);
}
...
bool PhysicsPassDispatcher::solveSliderJointTargetVelocities(
    Diligent::IDeviceContext *computeContext, const PhysicsSceneGpuState &sceneState)
{
    const std::uint32_t jointCount = sceneState.sliderDrivenJointCount();
    if (computeContext == nullptr)
    {
        return false;
    }
    ...
    return writeRigidJointDispatchConstants(computeContext, jointConstants) &&
           dispatchSolveSliderJointVelocityTargetsPass(
               computeContext, sceneState, persistentJoints.sliderDrivenJointIndicesBuffer,
               jointCount);
}
                                                        ...
                                                        const GpuRigidDispatchConstants &constants)
{
    ...
    const std::uint32_t hingeDrivenJointCount  = sceneState.hingeDrivenJointCount();
    const std::uint32_t sliderDrivenJointCount = sceneState.sliderDrivenJointCount();
    const auto &transient                      = sceneState.transientBuffers();
    const auto &persistent                     = sceneState.persistentRigidBodies();
    const std::array applyBindings{
        gpu::GpuBufferBinding{"PhysicsRigidDispatchConstantsBuffer", mRigidDispatchConstantsBuffer,
...
bool PhysicsPassDispatcher::recreateSceneBindingVariants()
{
    const bool recreated =
        mRigidPredictPass.forceRecreateAllVariants() &&
        ...
        mUpdateSurfaceTriangleNormalsPass.forceRecreateAllVariants() &&
        mUpdateSurfaceDeformableNormalsPass.forceRecreateAllVariants() &&
        mUpdateCurveRenderDataPass.forceRecreateAllVariants() &&
        mUpdateSurfaceBoundsPass.forceRecreateAllVariants() &&
        mFinalizeSurfaceBoundsPass.forceRecreateAllVariants() &&
        ...
        mSolveSphericalJointConstraintsPass.forceRecreateAllVariants() &&
        ...
        mClearSphericalJointConstraintStatePass.forceRecreateAllVariants() &&
        ...
        mUpdateHingeJointRuntimeStatePass.forceRecreateAllVariants() &&
        mUpdateSliderJointRuntimeStatePass.forceRecreateAllVariants() &&
    ...
    const std::array bindings{gpu::GpuBufferBinding{
        "PhysicsSolverConfigBuffer", mSolverConfigBuffer, Diligent::BUFFER_VIEW_SHADER_RESOURCE}};
